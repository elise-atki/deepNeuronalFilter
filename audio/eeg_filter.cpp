...
// FILES
	...
	while (!p300_infile.eof()) {
		...
		double p300trigger = 0; //what is in trigger?
		
		if (nullptr == tasksubdir) {
		  p300_infile >> inner_raw_data >> outer_raw_data >> p300trigger; //gets in the raw data from rawp300
		} else {
			std::string line;
			std::getline(p300_infile, line); //accepts string input from p300infile
			vector<string> row_values;
			split(line, '\t', row_values);
			if (row_values.size()>7) {
			  inner_raw_data = boost::lexical_cast<double>(row_values[7]); //converts string input to double no. val
			  outer_raw_data = boost::lexical_cast<double>(row_values[8]); ///why row 7&8?
			}
			p300trigger = 0;
		}
		...
		innertrigger_delayLine.push_back(p300trigger); //adds val in p300trig to end of intrigdelayline and sets ad delayp300trig
